The naming and decomposition of this element is a bit messed up.  The suggested approach is

http-component (repo/package name)
├── <http-message>        # Base: renders headers + body (shared by req/res)
├── <http-request>        # Request-line + http-message
├── <http-response>       # Status-line + http-message  
├── <http-transaction>    # Request + Response pair (what you call http-console)
├── <http-waterfall>      # List/timeline of transactions (keep as-is)
└── http-interceptor.js   # Utility for live capture (not a component)

Further refactor instructions follow
